...
const LobbyHeader = ({ isGuest = false }) => {
  ...
  // Fetch profile data if not a guest
  useEffect(() => {
    if (!isGuest) {
      const fetchProfileData = async () => {
        try {
          ...
          console.error('Error fetching profile data in header:', err);
        }
      };
      fetchProfileData();
    }
    ...
    <header className="z-30 flex items-center justify-between h-16 px-4 md:px-6 bg-white dark:bg-gray-800 shadow-sm">
      ...
      <div className="flex items-center space-x-1 sm:space-x-3">
        ...
        {isGuest ? (
          ...
          <div className="flex items-center space-x-3">
            {/* Fallback for when profile data is loading for a logged-in user */}
