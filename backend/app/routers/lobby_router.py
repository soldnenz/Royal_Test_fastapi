...
async def validate_lobby_access(lobby_id: str, user_id: str, required_status: str = None, is_guest: bool = False):
    ...
    if required_status and lobby["status"] != required_status:
        lobby = await db.lobbies.find_one({"_id": lobby_id}) # Повторная проверка
    ...
    # Получаем подписку напрямую из БД
    ...
    if is_guest:
        host_subscription = await get_subscription(lobby.get("host_id"))
...
async def get_online_users(
    ...
    """
    # This endpoint is deprecated and moved to multiplayer/participants_router.py
    # Please use /lobbies/{lobby_id}/participants instead.
    logger.warning(
        section=LogSection.SYSTEM,
        subsection=LogSubsection.SYSTEM.DEPRECATED,
        message=f"Вызван устаревший эндпоинт /lobbies/{lobby_id}/online-users"
    )
    raise HTTPException(status_code=410, detail="Этот эндпоинт устарел и будет удален. Используйте /lobbies/{lobby_id}/participants.")
...
async def create_lobby(lobby_data: LobbyCreate, request: Request = None, current_user: dict = Depends(get_current_actor)):
    """
    Создает новое лобби для прохождения теста с учетом подписки пользователя.
