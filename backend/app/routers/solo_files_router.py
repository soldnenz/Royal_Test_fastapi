...
async def get_question_media_secure(
    ...
    try:
        ...
        # Получаем медиа-файл из новой системы
        try:
            logger.info(
                section=LogSection.FILES,
            ...
            # Проверяем существование файла в новой системе медиа
            file_info = await media_manager.get_media_file(str(media_file_id), db)
            if not file_info:
                logger.error(
                    section=LogSection.FILES,
                    subsection=LogSubsection.FILES.GRIDFS,
                    message=f"Медиа-файл не найден в новой системе: файл ID {media_file_id} отсутствует в коллекции media_files"
                )
                raise HTTPException(
                    status_code=404,
            ...
            logger.info(
                section=LogSection.FILES,
                subsection=LogSubsection.FILES.GRIDFS,
                message=f"Медиа-файл найден в новой системе: имя '{file_info.get('original_filename', 'неизвестно')}', размер {file_info.get('file_size', 0)} байт"
            )
        ...
        try:
            ...
            if not file_info:
                logger.error(
                    section=LogSection.FILES,
                    subsection=LogSubsection.FILES.GRIDFS,
                    message=f"Дополнительный медиа-файл не найден в новой системе: файл ID {after_answer_media_id} отсутствует в коллекции media_files"
                )
                raise HTTPException(
                    status_code=404,
            ...
            logger.info(
                section=LogSection.FILES,
                subsection=LogSubsection.FILES.GRIDFS,
                message=f"Дополнительный медиа-файл найден в новой системе: имя '{file_info.get('original_filename', 'неизвестно')}', размер {file_info.get('file_size', 0)} байт"
            )
